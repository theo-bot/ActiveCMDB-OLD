...
sub index :Path :Args(0) {
    my ( $self, $c ) = @_;
	if ( cmdb_check_role($c,qw/importAdmin/) ) {
...
sub list :Local {
	...
	if ( cmdb_check_role($c,qw/importAdmin/) )
	{
		...
		my $start = $page * $rows;
		my @types = ();
		if ( cmdb_check_role($c,qw/deviceAdmin/) ) { push(@types,'object_type:Device'); }
		if ( cmdb_check_role($c, qw/siteAdmin/ ) ) { push(@types,'object_type:Site'); }
		
		if ( scalar(@types) > 0 ) {
			$query = sprintf("dt:cmdbImport AND (%s)",join(' OR ', @types));
		}
		$c->log->debug("Cloud query: $query");
	
		my @imports = cmdb_get_imports($order, $start, $rows, $query );
...
sub edit :Local {
	...
	if ( cmdb_check_role($c,qw/importAdmin/) )
	{
		...
		if ( defined($id) ) 
		{
			...
			$c->stash->{objTypes} = { cmdb_import_types($c) };
